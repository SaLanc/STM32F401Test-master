...
float MS5803_GetAltitudeBlocking()
{
    ...
    float pressureMbar;
    float Temperature;
    uint32_t TemperatureRaw = MS5803_GetTemperature_Blocking();
    uint32_t PressureRaw = MS5803_GetPressure_Blocking();
    ...
    volatile const int32_t dT = (TemperatureRaw) - ((int32_t)C5 << 8);
    Temperature = 2000 + (((int64_t)dT * C6) >> 23);
...
uint32_t MS5803_GetTemperature_Blocking()
{
    ...
    HAL_Delay(5);
    uint32_t _temperature = readMS5803AdcResults();
    return _temperature;
}
...
uint32_t MS5803_GetPressure_Blocking()
{
    ...
    HAL_Delay(5);
    uint32_t _pressure = readMS5803AdcResults();
    return _pressure;
}
